...
const useCompilations = (track: TrackBasic, dbService: DatabaseService) => {
  ...
  const isGatheringFetching = computed(() => isCreating.value || isUpdating.value || isCompilationsFetching.value)

  const { data: initialCompilations, isFetching: isCompilationsFetching } = useGetList<CompilationBasic>(
    compilationsConfig,
    dbService,
  ...
  const { createdEntity, createEntity, isCreating } = useCreateEntity<
    ListPageResponse<CompilationBasic>,
    NewGatheringPayload
  >(compilationEntityKey, dbService)
  ...
  watch([initialCompilations, updatedEntity, createdEntity], ([a, b, c], [oldA, oldB, oldC]) => {
    if (!!a && a !== oldA) {
